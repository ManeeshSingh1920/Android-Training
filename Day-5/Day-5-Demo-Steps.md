...
1. 
Inside the Demos folder we have the  "many" folders
copy and paste inside the vendor/xrda3 folder and build it:	
...
mmm vendor/xrda3/simple-hal/aidl \
    vendor/xrda3/simple-hal/service \
    vendor/xrda3/simple-service \
    vendor/xrda3/simple-manager \
    vendor/xrda3/simple-manager-app \
    vendor/xrda3/light-hal-aidl

# Regenerate images so new paths/allowlist are applied
...
2. 
# Now push to Cuttlefish -
	adb root   // Start adb in root
	adb disable-verity  // disabling dm-verityso that we can access read only partitions..
	adb reboot  // reboot
	adb remount // remount all the partiions
	adb sync system  // sync system partition
	adb sync vendor  // sync vendor partition
...
3. 
# check: rc & binary Should exist at below path which is defined in android.bp and .rc file of HAL service
adb shell ls -l /system/bin/vendor.xrda3.simple-hal-service
adb shell ls -l /system/etc/init/simple-hal-xrda3.rc
...
4. 
# Restart / Start App serice.. 
adb shell am start -S -n com.example.simplemanagerapp/.SimpleManagerActivity


# check HAL service is working or not... 
adb shell ps -A | grep simple-hal

adb logcat | grep -i "Simple HAL"


Incase HAL is not started start it manually - 

adb shell
xrda3car:/ # su
xrda3car:/ # /system/bin/vendor.xrda3.simple-hal-service &
xrda3car:/ # logcat | grep -i "simple HAL"


5. 
# Testing
In another terminal open - 
adb logcat | grep simple

Now try add and string from APp and see if you are gettign the proper results or not.....
Now chekc logcat logs... 

Now check serice list => adb shell service list | grep ISimple
...
==================================================================================================================================================================
																...
																Sepolicy audit2allow
																
1. Find the correct sepolicy file in AOSP	
   find out/target/product/xrda3car -type f -name "sepolicy"
  You’ll see one or more paths, for example (these are examples, your exact one may differ):
  out/target/product/xrda3car/obj/ETC/sepolicy_intermediates/sepolicy

  or out/target/product/xrda3car/root/sepolicy

  or out/target/product/xrda3car/vendor/etc/selinux/vendor_sepolicy.cil_intermediates/sepolicy etc.	
  
2.   									
  Then do -
  POLICY=out/target/product/xrda3car/recovery/root/sepolicy
...
4. 
  Now use below command to genrate sepolicy for the given error. 
  audit2allow -p "$POLICY" -i denial.txt > policy.te
  
  denial.txt => input file inwhich u r adding sepolicy errors..  name can be anything... 
  policy.te  => output file in which u will see sepolicy rule.... name can be anything... 
...
==================================================================================================================================================================
																...
																HIDL Demo-1 (On telechip HW)
																 
1. Follow the steps given in 	
   HIDL_DEMO_Android10_12Dec_Telechips8050  folder readme and pdf files.... 
...
==================================================================================================================================================================   
																...
																HIDL Demo-1 (On AOSP 14)

# In this demo we will do HIDl to AIDL converstion tool

#NOTE : we have already created a HIDL HAL named => simple 
#       We need to migrate it to AIDL interface 
#       Best way to use hidl2aidl tool 

																 
1. Step 1: Make build hidl2aidl =>

#  $ which hidl2aidl
   If it return nothing then build the hidl2aidl tool using below command 
...
2. Step 2 Delete old aidl folder
From AOSP top:
#  $ rm -rf vendor/xrda3/simple/aidl
#  $ mkdir -p vendor/xrda3/simple/aidl



3. Step 3: Regenerate aidl folder
#  $ hidl2aidl -r vendor.xrda3:vendor/xrda3 -o vendor/xrda3/simple/aidl  vendor.xrda3.simple@1.0

    -r vendor.xrda3:vendor/xrda3     Tells the tool:- “Any HIDL package starting with vendor.xrda3.* is located under vendor/xrda3/”
    -o vendor/xrda3/simple/aidl      Tells the tool:-  Where converted .aidl files + conversion.log go
       vendor.xrda3.simple@1.0       This is the FQNAME, exactly like hidl-gen



# What is FQNAME?
FQNAME = Fully Qualified Name (full “package@version + type/interface” identifier).



In our case, our ISimple.hal starts with:
# package vendor.xrda3.simple@1.0;
# interface ISimple { ... }



So our FQNAME is:
# vendor.xrda3.simple@1.0 (whole package), or
# vendor.xrda3.simple@1.0::ISimple (just the interface)
...
# In this demo we will do HIDL-gen tool to generate HIDL C++ and Android.bp boilerplate 



1. Source-of-truth files (DO NOT delete)

These are written by us:

# IEcho.hal
# types.hal

These files define the API.





2. Typical autogenrated files (from our Echo folder):

# Echo.h
# Echo.cpp
# service.cpp
# android.hardware.echo@1.0-service.rc
# Android.bp

These are boilerplate skeletons.

These can be deleted and regenerated
But only if we have NOT customized them	


3. Generate C++ implementation 

# hidl-gen -o vendor/xrda3/echo/1.0/default -Lc++-impl -r android.hardware:vendor/xrda3 android.hardware.echo@1.0


4. Generate Android.bp implementation 

# hidl-gen -o vendor/xrda3/echo/1.0/default -Landroidbp-impl -r android.hardware:vendor/xrda3 android.hardware.echo@1.0::
	
5. verify 

# tree hardware/interfaces/echo/1.0/default
